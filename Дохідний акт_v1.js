...
function setPropTypeSigning() {
  if (EdocsApi.getAttributeValue("edocsCreatedByExtSys").value == "vchasno") {
    ...
    if (
      EdocsApi.getAttributeValue("TypeSigning").value !=
      "Електронне підписання Вчасно"
    )
...
function setAccountantApproval() {
  ...
  if (
    AccountantApproval != EdocsApi.getAttributeValue("AccountantApproval").value
  )
    EdocsApi.setAttributeValue({
...
function onChangeContractorId() {
  ...
  if (ContractorId) {
    var edocsGETCONTRACTOR = EdocsApi.runExternalFunction(
      "1C",
      "edocsGETCONTRACTOR?format=json&CONTRACTORID=" +
        ContractorId.split("|")[0] +
        "&CONTRACTORTYPE=" +
        ContractorId.split("|")[1] +
        "&MAXRESULTCOUNT=200",
      null,
      "get"
    );
    if (edocsGETCONTRACTOR != null || edocsGETCONTRACTOR.data != null) {
      ...
      if (
        edocsGETCONTRACTOR.data.authorisedPersons &&
        edocsGETCONTRACTOR.data.authorisedPersons.length > 0
      ) {
        EdocsApi.setAttributeValue({
          code: "fullNane",
...
function onChangePercentagePDV() {
  if (
    EdocsApi.getAttributeValue("PercentagePDV").value &&
    EdocsApi.getAttributeValue("PaymentGross").value
  )
    setSumPDVControl();
  setSumHundreds();
}
...
function setPriceServices() {
  if (
    EdocsApi.getAttributeValue("QuantityService").value &&
    EdocsApi.getAttributeValue("SumNotPDV").value
  ) {
    EdocsApi.setAttributeValue({
      code: "PriceServices",
      value: (
        Number(EdocsApi.getAttributeValue("SumNotPDV").value) /
        Number(EdocsApi.getAttributeValue("QuantityService").value)
      ).toFixed(2),
      text: null,
    });
  }
...
function setSumHundreds() {
  ...
  EdocsApi.setAttributeValue({
    ...
    value: validationOfSumSigns(
      EdocsApi.getAttributeValue("PriceServices").value
    ),
  });
}
...
function sendToCreate1Cdoc() {
  var methodData = {
    ...
    attributes: [
...

      {
        ...
        value: EdocsApi.getAttributeValue("edocsIncomeDocumentDate").value
          ? moment(
              new Date(
                EdocsApi.getAttributeValue("edocsIncomeDocumentDate").value
              )
            ).format("YYYY-MM-DD")
          : null,
      },
  ...
  if (response.data) {
    if (response.data.docId) {
      ...
      EdocsApi.message(
        "Запис успішно створений. Ідентифікатор:" + response.data.docId
      );
    } else if (response.data.error) {
      if (
        response.data.error.validationErrors &&
        response.data.error.validationErrors.length > 0
      ) {
        var errorMessage = "";
        for (var i = 0; i < response.data.error.validationErrors.length; i++) {
          errorMessage +=
            response.data.error.validationErrors[i].message + "; ";
        }
...
function myOnChangeTypeSigning(flag) {
  ...
  if (
    TypeSigning == "Електронне підписання Вчасно" ||
    TypeSigning == "Електронне підписання е-sign"
  ) {
    ...
    if (TypeSigning == "Електронне підписання Вчасно" && flag)
      EdocsApi.message(
        "На підписання контрагенту буде відправлено лише контент першої вкладки документа"
      );
  } else {
    setControlRequired("CounterpartyEmailAuthorisedPersons", false);
...
function setProjectManager() {
  if (
    EdocsApi.getAttributeValue("Manager")?.value ==
    EdocsApi.getAttributeValue("ProjectManager")?.value
  ) {
    ...
    if (
      Number(EdocsApi.getAttributeValue("ProjectManagerManager").value) != 0 ||
      !EdocsApi.getAttributeValue("ProjectManagerManager").value
    )
      EdocsApi.setAttributeValue({
        code: "ProjectManagerManager",
        value: 0,
  ...
  doc = {
    ...
    parties: [
      ...
      },
      {
        ...
        contactPersonEmail: EdocsApi.getAttributeValue(
          "CounterpartyEmailAuthorisedPersons"
        ).value,
