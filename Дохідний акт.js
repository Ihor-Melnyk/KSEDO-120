...
function setPropTypeSigning(){
    if(EdocsApi.getAttributeValue('edocsCreatedByExtSys').value == 'vchasno'){
        ...
        if(EdocsApi.getAttributeValue('TypeSigning').value != 'Електронне підписання Вчасно')
            EdocsApi.setAttributeValue({code: "TypeSigning", value: "Електронне підписання Вчасно", text: null});
    }
}
...
function onChangeBasicContract(){
    ...
    if(BasicContract){
        ...
        if(item && item.attributes){
            EdocsApi.setAttributeValue({code: 'NumberDateBasicAgreement', value: item.attributes.find(x=>x.Contract_Number)?.Contract_Number, text: null});
            var dateStr = item.attributes.find(x=>x.Contract_Date)?.Contract_Date;
            if(dateStr && dateStr != ''){
                EdocsApi.setAttributeValue({code: 'DateBasicAgreement', value: new Date(dateStr).toISOString(), text: null});       
                setDateSTR('DateBasicAgreement', 'DateBasicAgreementText');    
            }
        }
    }
    else{
        EdocsApi.setAttributeValue({code: 'NumberDateBasicAgreement', value: null, text: null});
        EdocsApi.setAttributeValue({code: 'DateBasicAgreement', value: null, text: null});
    }
}
...
function setAccountantApproval(){
    ...
    if(AccountantApproval != EdocsApi.getAttributeValue('AccountantApproval').value)
        EdocsApi.setAttributeValue({code: 'AccountantApproval', value: AccountantApproval, text: null});
}
...
setAccountantApproval();
 ...
 EdocsApi.setAttributeValue({code: 'AccountantApproval', value:  null, text: null});
}
}
        ...
        if(item){
            if(item.attributes[1].FM == '' || !item.attributes[1].FM){    
               ...
               //throw 'Відсутній фінінсовий менеджер';
                ...
                if(FM)
                    EdocsApi.setAttributeValue({code: 'FinancialManager', value: FM.employeeId, text: null});
            }
            ...
            if(item.attributes[2].PM == '' || !item.attributes[2].PM){
                ...
                if(PM)
                    EdocsApi.setAttributeValue({code: 'ProjectManager', value: PM.employeeId, text: null});
            }
         if(item.attributes[3].fullname == '' || !item.attributes[3].fullname){
                ...
                //throw 'Відсутній проєктний менеджер';
                    ...
                    EdocsApi.setAttributeValue({code: 'ServicesProject', value: item.attributes[3].fullname, text: null});
            }
        }
    }

    else{
        EdocsApi.setAttributeValue({code: 'FinancialManager', value: null, text: null});
        EdocsApi.setAttributeValue({code: 'ProjectManager', value: null, text: null});
        EdocsApi.setAttributeValue({code: 'ServicesProject', value: null, text: null});
    }
}
...
function onChangeContractorId() {
    ...
    if (ContractorId) {
        ...
        if (edocsGETCONTRACTOR != null || edocsGETCONTRACTOR.data != null) {
            EdocsApi.setAttributeValue({code: 'StateRegDate', value: edocsGETCONTRACTOR.data.StateRegDate, text: null});
            EdocsApi.setAttributeValue({code: 'StateRegNumber', value: edocsGETCONTRACTOR.data.StateRegNumber, text: null});
            ...
            if(edocsGETCONTRACTOR.data.authorisedPersons && edocsGETCONTRACTOR.data.authorisedPersons.length > 0){
                EdocsApi.setAttributeValue({code: 'fullNane', value: edocsGETCONTRACTOR.data.authorisedPersons[0].fullNane, text: null});
                ...
                EdocsApi.setAttributeValue({code: 'position', value: edocsGETCONTRACTOR.data.authorisedPersons[0].position, text: null});
...
function onChangeOrganizationID() {
    ...
    if (OrganizationID) {
        ...
        if (edocsGETCONTRACTOR != null || edocsGETCONTRACTOR.data != null) {
            EdocsApi.setAttributeValue({code: 'Mark3', value: edocsGETCONTRACTOR.data.Mark, text: null});
            if(edocsGETCONTRACTOR.data.authorisedPersons && edocsGETCONTRACTOR.data.authorisedPersons.length > 0){
                EdocsApi.setAttributeValue({code: 'ORGNameSignatory', value: edocsGETCONTRACTOR.data.authorisedPersons[0].fullNane, text: null});
                EdocsApi.setAttributeValue({code: 'OrgNameSignatory3', value: edocsGETCONTRACTOR.data.authorisedPersons[0].nameGenitive, text: null});
                EdocsApi.setAttributeValue({code: 'OrgPositionSignatory3', value: edocsGETCONTRACTOR.data.authorisedPersons[0].positionGenitive, text: null});
                EdocsApi.setAttributeValue({code: 'OrgPositionSignatory', value: edocsGETCONTRACTOR.data.authorisedPersons[0].position, text: null});
                EdocsApi.setAttributeValue({code: 'OrgBasis', value: edocsGETCONTRACTOR.data.authorisedPersons[0].actingUnderThe, text: null});
                EdocsApi.setAttributeValue({code: 'OrgAuthorisedPersonsFirstName', value: edocsGETCONTRACTOR.data.authorisedPersons[0].FirstName, text: null});
                EdocsApi.setAttributeValue({code: 'OrgAuthorisedPersonsLastName', value: edocsGETCONTRACTOR.data.authorisedPersons[0].LastName, text: null});
                EdocsApi.setAttributeValue({code: 'EmailAuthorisedPersons', value: edocsGETCONTRACTOR.data.authorisedPersons[0].Email, text: null});
...
function onChangePercentagePDV(){
    if(EdocsApi.getAttributeValue('PercentagePDV').value && EdocsApi.getAttributeValue('PaymentGross').value)
        setSumPDVControl();
}
...
function setSumPDVControl(){
    ...
    if(PercentagePDV == 0){
        EdocsApi.setAttributeValue({code: 'SumPDV', value: PaymentGross, text: null});
        EdocsApi.setAttributeValue({code: 'SumPDV2', value: 0.00, text: null});
        EdocsApi.setAttributeValue({code: 'SumNotPDV', value: PaymentGross, text: null});
        setPriceServices();
    }
    else if(PercentagePDV == 20){
        EdocsApi.setAttributeValue({code: 'SumPDV', value: PaymentGross, text: null});
        EdocsApi.setAttributeValue({code: 'SumPDV2', value: (PaymentGross/6).toFixed(2), text: null});
        EdocsApi.setAttributeValue({code: 'SumNotPDV', value: PaymentGross-PaymentGross/6, text: null});
        setPriceServices();
    }
    onChangeSumNotPDV();
    onChangeSumPDV();
...
function setPriceServices(){
    if(EdocsApi.getAttributeValue('QuantityService').value && EdocsApi.getAttributeValue('SumNotPDV').value){
        EdocsApi.setAttributeValue({code: 'PriceServices', value: (Number(EdocsApi.getAttributeValue('SumNotPDV').value)/Number(EdocsApi.getAttributeValue('QuantityService').value)).toFixed(2), text: null});
    }
}
...
function setNumberToWords(NumberCode, TextCode){
    if(CurrentDocument.isDraft){
        ...
        if(value){
           EdocsApi.setAttributeValue({code: TextCode, value:  EdocsApi.numberToCurrency(Number(value).toFixed(2), 'uk', 'UAH'), text: null});
            EdocsApi.setAttributeValue({code: NumberCode, value:  Number(value).toFixed(2), text: null});
        }
        else{
...
function onChangeSumNotPDV(){
    ...
    value: EdocsApi.getAttributeValue("SumNotPDV").value,
  });
}
function onChangeSumPDV2(){
    ...
    value: EdocsApi.getAttributeValue("SumPDV2").value,
  });
}
...
function onChangeSumPDV(){
    ...
    EdocsApi.setAttributeValue({
    code: "SumPDVHundreds",
    value: EdocsApi.getAttributeValue("SumPDV").value,
  });
}
...
function onChangePriceServices() {
  EdocsApi.setAttributeValue({
    ...
    value: EdocsApi.getAttributeValue("PriceServices").value,
  });
}
...
function sendToCreate1Cdoc(){
        ...
        attributes:[
          ...
          EdocsApi.getAttributeValue('OrgPositionSignatory'),
           ...
           EdocsApi.getAttributeValue('Services'), 
            ...
            { code: 'edocsIncomeDocumentDate', value: ((EdocsApi.getAttributeValue('edocsIncomeDocumentDate').value) ? (moment(new Date(EdocsApi.getAttributeValue('edocsIncomeDocumentDate').value)).format('YYYY-MM-DD')) : null) },
    ...
    var response = EdocsApi.extSysCreateDoc(
                    externalSystemCode = '1C', 
                    externalSystemMethod = 'edocsCreateDoc',
                    methodData = methodData,
                    methodFiles = 'fixedFiles',
                    methodSignFiles = true);

    if (response.data) {
        if (response.data.docId) {     
            EdocsApi.setAttributeValue({code: 'DocumentID1C', value: response.data.docId, text: null});           
...
function setSTRDates(){
    ...
    if(EdocsApi.getAttributeValue('edocsIncomeDocumentDate').value){
        EdocsApi.setAttributeValue({code: 'RegDateText', value: new Date(EdocsApi.getAttributeValue('edocsIncomeDocumentDate').value).toLocaleDateString('uk-UA', {year: 'numeric', month: 'long', day: 'numeric' }), text: null});
    }
    else{
        EdocsApi.setAttributeValue({code: 'RegDateText', value: null, text: null});
    }
}
...
function myOnChangeTypeSigning(flag){
    ...
    if(TypeSigning == 'Електронне підписання Вчасно' || TypeSigning == 'Електронне підписання е-sign'){
        ...
        if(TypeSigning == 'Електронне підписання Вчасно' && flag)
            EdocsApi.message('На підписання контрагенту буде відправлено лише контент першої вкладки документа');
    }
    else{
        setControlRequired('CounterpartyEmailAuthorisedPersons', false);
...
function onChangeCounterpartyEDRPOU(){
    ...
    EdocsApi.setAttributeValue({code: 'BasicContract', value: null, text: null});
}

function onSearchBasicContract(searchRequest){
    if(!EdocsApi.getAttributeValue('OrgEDRPOU').value)
        throw 'Спочатку оберіть домашню організацію';
    if(!EdocsApi.getAttributeValue('CounterpartyEDRPOU').value)
        throw 'Спочатку оберіть контрагента';

        searchRequest.response = EdocsApi.getDictionaryData('Договори', '', [{attributeCode: 'OrgEDRPOU', value: EdocsApi.getAttributeValue('OrgEDRPOU').value}, {attributeCode: 'CounterpartyEDRPOU', value: EdocsApi.getAttributeValue('CounterpartyEDRPOU').value}]);
}
...
function setProjectManager(){    
    if(EdocsApi.getAttributeValue('Manager')?.value == EdocsApi.getAttributeValue('ProjectManager')?.value){
        if(Number(EdocsApi.getAttributeValue('ProjectManagerManager').value) != 1)
            EdocsApi.setAttributeValue({code: 'ProjectManagerManager', value: 1, text: null});
    }
    else{
        if(Number(EdocsApi.getAttributeValue('ProjectManagerManager').value) != 0 || !EdocsApi.getAttributeValue('ProjectManagerManager').value)
            EdocsApi.setAttributeValue({code: 'ProjectManagerManager', value: 0, text: null});
    }
}
...
function onTaskExecuteSendOutDoc(routeStage) {
    if (routeStage.executionResult != 'rejected') {
        ...
        var methodData = {
            extSysDocId: CurrentDocument.id, ExtSysDocVersion: CurrentDocument.version
        };
